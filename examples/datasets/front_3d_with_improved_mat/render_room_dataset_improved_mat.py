...
if __name__ == '__main__':
    ...
    with open(front_json, 'r') as f:
        front_json_data = json.load(f)
    # bproc.renderer.enable_depth_output(activate_antialiasing=False)
    for room_id in range(len(front_json_data["scene"]["room"])):
        '''Pass already generated scenes.'''
        scene_output_folder = output_folder.joinpath(f"{scene_name}_room_{room_id:02d}")
